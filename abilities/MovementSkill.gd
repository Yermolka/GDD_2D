...
## Method for AI
func set_tp_target(target_pos: Vector2) -> void:
...
func activate(event: ActivationEvent) -> void:
	...
	match movement_type:
		...
		1:
			...
			var dir: Vector2 = caster.velocity
			if dir.length() == 0.0:
				dir = Vector2(1, 0)
			else:
				dir = dir.normalized()
...
func _dash(caster: Entity, dir: Vector2) -> void:
...
func can_activate(event: ActivationEvent) -> bool:
	...
	match movement_type:
		0:
			if caster is Player:
				tp_target = caster.get_global_mouse_position()
